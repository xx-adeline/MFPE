...
from model.single_flow import calculate_sim_neg_for_test
...
def test(opt, epoch, network, img_dataloader, txt_dataloader, best, return_flag=True):
    ...
    for times, [image, label] in enumerate(img_dataloader):
        ...
        with torch.no_grad():
            img_global_i, img_local_i, img_mn_i = network.img_embedding(image)

        image_feature_global.append(img_global_i)
        image_feature_local.append(img_local_i)
        image_feature_mn.append(img_mn_i)
    ...
    image_feature_mn = torch.cat(image_feature_mn, 0)
    ...
    for times, [label, caption_code, caption_length, caption_mask, _] in enumerate(txt_dataloader):
        label = label.to(opt.device)
        ...
        with torch.no_grad():
            text_global_i, text_local_i, text_mn_i = network.txt_embedding(caption_code, caption_mask, caption_length)

        text_feature_local.append(text_local_i)
        text_feature_global.append(text_global_i)
        text_feature_mn.append(text_mn_i)
    ...
    text_feature_mn = torch.cat(text_feature_mn, 0)
    ...
    similarity_neg = calculate_sim_neg_for_test(image_feature_mn, text_feature_mn)

    similarity_neg_local = similarity_local + similarity_neg
    similarity_all = similarity_local + similarity_global
    similarity_neg_all = similarity_local + similarity_global + similarity_neg_local
    ...
    best = write_result(similarity_neg_all, img_labels, txt_labels, 'similarity_neg_all:',
                        txt_root, best_txt_root, epoch, best)
